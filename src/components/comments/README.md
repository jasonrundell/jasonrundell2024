...
Client-side comment threads attached to posts and projects. All data lives in Supabase; the API layer in `src/app/api/comments/` enforces row-level security and rate limiting.
...
Comments are fetched flat and rendered in a single list (no nested tree UI). Replies appear as siblings with a `parent_id` set.
...
`CommentsSection` now surfaces a visible error state when the fetch fails — it does **not** silently return an empty list. See `CONVENTIONS.md` for the no-silent-fallbacks rule.
