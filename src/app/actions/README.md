...
Each file in this folder contains a **single** Next.js server action (`'use server'`) grouped by feature. All server actions are re-exported from `src/app/actions.ts` for backward-compatible import paths.
...
4. Add a colocated `<feature>.test.ts` covering at least: validation errors, rate-limit, success path.
