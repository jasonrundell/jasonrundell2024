...
function getBaseUrl() {
  ...
  return process.env.NEXT_PUBLIC_URL || `http://localhost:${process.env.PORT || 3000}`
}
...
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    try {
      if (!existingUser) {
        console.log('Attempting to create new user with data:', JSON.stringify(userData, null, 2));
        ...
        console.log('Attempting to update existing user with data:', JSON.stringify(userData, null, 2));
    ...
    // Sign in with OAuth
    const { data: authData, error: authError } = await supabaseAuth.auth.signInWithOAuth({
      provider: 'github',
      options: {
        redirectTo,
        queryParams: {
    ...
    if (authError) {
      ...
      return NextResponse.redirect(new URL(`/sign-in?error=auth_session&details=${encodeURIComponent(authError.message)}`, baseUrl))
    }

    if (!authData.url) {
      console.error('No URL returned from auth session creation')
      return NextResponse.redirect(new URL('/sign-in?error=no_auth_url', baseUrl))
    }
    ...
    return NextResponse.redirect(
      new URL(`/sign-in?error=server_error&details=${encodeURIComponent(errorMessage)}`, baseUrl)
    )
  }
}
