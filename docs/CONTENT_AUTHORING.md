...
This site manages all editorial content as local MDX files in the `content/`
directory. No CMS account or API key is required to add or edit content.
...
Images live **co-located** with their MDX file. They are automatically synced to
`public/content/` before every `dev` and `build` run via
`scripts/sync-content-images.js`, which serves them at
`/content/<type>/<slug>/...`.
...
The `predev` script runs `sync-content-images` automatically - no manual step
needed.
...
2. Write the frontmatter and body in `content/posts/my-new-post/index.mdx`:
   ...
   Your post body in Markdown. GFM features (tables, task lists, strikethrough)
   are supported.
...
Each skill needs an `id`, `name`, and `category`. The `id` can be any unique
string.
...
The `quote` field is a plain string (markdown-in-a-YAML-string is not rendered;
keep quotes to plain prose).
...
- The sync script (`scripts/sync-content-images.js`) runs automatically; never
  commit to `public/content/` directly

---

## Rebuilding after content changes

Content is read at **build time** (static generation). After adding or editing
content:
...
The migration from Contentful was a one-time operation. The migration script has
been removed. All content now lives in `content/` and is managed via MDX files
